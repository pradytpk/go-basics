...
func (lot *ParkingLot) vacantSpace(spaceNum int) {
	lot.spaces[spaceNum-1].occupied = false
}
func main() {
	lot := ParkingLot{spaces: make([]Space, 4)}
	fmt.Println("Intial occupied:", lot)
	lot.occupySpace(1)
	occupySpace(&lot, 2)
	fmt.Println("After occupied:", lot)

	lot.vacantSpace(2)
	fmt.Println("After vacantSpace:", lot)
...
//--Summary:
//  Implement receiver functions to create stat modifications for a video game character.
